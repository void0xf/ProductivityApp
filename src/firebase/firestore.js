...
export async function addDataToFirebase(firestore, uid, tasks, completedTask, notes) {
  const jsonString = JSON.stringify(tasks);
  const userCollectionRef = collection(firestore, 'users');
  const userDocRef = doc(userCollectionRef, uid); // Use the UID as the document ID
  await setDoc(userDocRef, { tasks,  completedTask, notes }); // Set the document data to { tasks }
}
