...
const Upcoming = () => {
  ...
  useEffect(() => {
    setTodayTasks(
      getTasksForToday(state.tasks, filterState.listFilter, searchState.search)
    );
    setTommorowTasks(
      getTasksForTommorow(
        state.tasks,
  ...
  return (
    <div className="sm: mx-4 h-screen sm:overflow-y-scroll sm:scroll-m-0 sm:no-scrollbar">
      <ComputerTitleCard
        name={"Upcoming"}
        numberOfNotifictaions={
          thisWeekTasks.length > 0 ? thisWeekTasks.length : "-"
        }
      />
